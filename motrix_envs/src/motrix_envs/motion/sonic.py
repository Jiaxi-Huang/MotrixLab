...
class SonicMotionClip(WbtMotionClip):
    ...
    def from_motion(
        ...
        missing = [name for name in ("smpl_joints", "smpl_root_quat") if name not in motion.extensions]
        if missing:
            raise ValueError(
                f"SONIC motion file is missing required SMPL extensions {missing}; "
                "pack the clip with the SONIC packer so both encoders receive reference data"
            )
        # The command advances exactly one clip frame per 0.02 s control step,
        # so anything but 50 Hz would silently play the reference at the wrong
        # speed. The packed-store path enforces the same contract.
        if motion.fps != 50:
            raise ValueError(
                f"SONIC motion data must be 50 Hz to match the ctrl step, got {motion.fps} Hz; "
                "resample the clip before use"
            )
        ...
        smpl_joints = np.ascontiguousarray(motion.extensions["smpl_joints"])
        smpl_root_quat = np.ascontiguousarray(motion.extensions["smpl_root_quat"])
