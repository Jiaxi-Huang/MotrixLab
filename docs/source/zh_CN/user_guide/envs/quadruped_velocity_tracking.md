...
MotrixLab 已为下列机器人提供平地、程序化粗糙高度场以及台阶地形高度场等组合配置。所有 Env ID
都注册到同一个 `QuadrupedWalkTask`，但分别选择一份完整配置。点击训练曲线缩略图可查看 SVG 大图。
...
| `go1-walk-stairs` | Unitree Go1 | 程序化金字塔台阶平台与坑 | `motrix.fastsac`、`rslrl.ppo`、`skrl.ppo` | — |
...
| `go2-walk-stairs` | Unitree Go2 | 程序化金字塔台阶平台与坑 | `motrix.fastsac`、`rslrl.ppo`、`skrl.ppo` | — |
...
同一机器人的各地形配置都基于对应的平地任务。粗糙地形仅替换高度场；台阶任务还会把 reset 放到台阶结构中心，
并放宽垂直速度惩罚，避免上下台阶所需的持续垂直运动被当作失败动作。表中的曲线展示粗糙地形任务的训练过程：
横轴为累计环境步数，纵轴为平均 Episode 回报，可用于观察策略的学习进展和收敛趋势。整体回报在训练早期快速提升
并趋于稳定，说明策略能够较快学会持续的粗糙地形行走。在当前训练配置与硬件条件下，本页展示的任务约
1 ～ 2 分钟内即可收敛，体现出并行仿真与训练流程的高吞吐效率。
