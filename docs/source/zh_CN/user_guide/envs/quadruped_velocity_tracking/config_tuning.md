...
传感器映射、动作缩放、奖励目标和地形。先建立一份完整的平地配置，再让各地形变体继承它，可以确保任务只在预期
字段上产生差异。
...
内置平地与粗糙地形配置都使用 `spawn_xy_range=4.0`。各地形变体继承完整的平地配置，只覆盖场景和地形相关字段：
粗糙地形将 `FlatTerrainCfg` 替换为 `QuadrupedWalkRoughSceneAssetsCfg` 与 `HFieldTerrainCfg`；台阶地形还会使用
固定的台阶结构出生槽位。重置时环境在出生点附近九个采样点中取最高地形高度并抬高基座；运行时的机体高度奖励也
以局部地形为参考。
